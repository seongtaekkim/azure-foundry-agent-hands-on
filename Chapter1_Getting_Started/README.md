...
이 장에는 실습 파일이 모두 9개 있지만, **6시간 과정에서 1장에 배정된 시간은 약 40분**입니다(전체 진행표는 루트 [README의 “6시간 Hands-on 진행표”](../README.md#6시간-hands-on-진행표) 참고). 그래서 이 장에서 **필수로 실행할 파일은 아래 두 개**이며, 번호 순서대로 실행합니다.
...
이 두 파일이면 1장 학습 목표(첫 모델 호출, multi-turn 대화)를 모두 확인할 수 있습니다. 끝나면 곧바로 [2장](../Chapter2_Foundry_Fundamentals/README.md)으로 넘어가세요.

나머지 7개 파일은 프롬프트 패턴을 더 연습하기 위한 **선택 실습**입니다. 시간이 남을 때 관심 있는 주제만 골라 실행해도 됩니다.
